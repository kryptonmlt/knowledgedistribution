...
Repeat until convergence\;
    \ForEach{$j$}{
$theta_j := \theta_j - \alpha\sum_{i=1}^{m}(y^{(i)}-h_\theta(x^{(i)}))x_j^{(i)}$
}
...
To use our reliability algorithm, the clusters, $errors$ and $used$ values need to be transferred along with the regression models. The clusters need not be updated as frequently as the model. Thus, they make use of a separate $GAMMA$ error threshold and are only sent when the total movement of the cluster centroids surpasses this threshold. As an optimization the pairs of $errors$ and $used$ values can be multiplied together to be send as 1 value instead of 2. Using $K=3$ centroids, we would have 6 values (each centroid has $(x,y)$) and 3 other values representing the $errors$ and $used$ counters. The message containing only the regression model consists of only 3 values but when adding the quantized information it adds up to a total of 12 values. This is larger than the simple model message, thus $GAMMA$ should be adjusted so as achieve network efficiency.
...
\paragraph{History of Models:}In the evaluation chapter we noticed that as we increased $THETA$ we were getting better query performance. Thus, this would suggest that we can store a history of models instead of just one model for each sensor at the concentrator. 
\paragraph{Concentrator teaching:}In our algorithm we were transmitting the $error$ and $used$ values along with the centroids with the intention of demonstrating which are the most reliable centroids. An alternative approach would be to occasionally transmit raw data to the concentrator. This raw data at the concentrator can be used to teach the concentrator which subset of models to use for various circumstances in the input space.
