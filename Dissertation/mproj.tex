...
In this chapter we describe the studies and research works that have been carried out in the field of energy efficient sensor networks. Additionally we shall delve into other areas such as ensemble learning and edge computing. We then evaluate the other works whilst comparing them with our approach.

\section{Wireless Networks}
\cite{teen}
\cite{adaptiveProtocols}
\cite{timeOptimizedContextual}
\cite{CooperativeDiversity}
\cite{distributedEnergyDistribution}
\cite{surveyMilitary}
\cite{SmartSensorWeb}
\cite{energyAwareRouting}
\cite{directedDiffusion}
\cite{energyEfficientDataManagement}
\cite{informationCollection}
\cite{optimalSelective}

\section{Edge Computing}
\cite{authenticatingQueries}
\cite{fogComputing}
\cite{replicatingInternetApplications}
