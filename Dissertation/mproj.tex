...
\paragraph{TODO: MICA CALCULATION INCLUDE COMPUTATION}

\subsection{Including Reliability}
To use our reliability algorithm, the clusters, $errors$ and $used$ values need to be transferred along with the regression models. The clusters need not be updated as frequently as the model. Thus, they should only be transmitted when an error threshold $GAMMA$ is exceeded. As an optimization the pairs of $errors$ and $used$ values can be multiplied together to be sent as 1 value instead of 2. Using $K=3$ centroids, we have 6 values (each centroid has $(x,y)$) and 3 other values representing the $errors$ and $used$ counters. The message that contains only the regression model consists of 3 values but when adding the quantized information it adds up to a total of 12 values. This is larger than the simple model message, thus $GAMMA$ should be adjusted so as to achieve network efficiency. We analyzed the impact of altering $GAMMA$, from transmitting the clusters for each minimal change to only transmitting once after the learning stage.
